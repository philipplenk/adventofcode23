...
int main(int argc, char* argv[])
{
	...
	const auto energize_from = [&](point2d start, direction initial_direction) -> int
	{
		std::vector<beam> beams;
		beams.push_back({start,initial_direction});
		...
		return std::ranges::count_if(seen,[](auto v){ return v>0; });
	};
	...
	{
		max = std::max(max,energize_from({0,y},direction::right));
		max = std::max(max,energize_from({width-1,y},direction::left));
	}

	for(int x=0;x<width;++x)
	{
		max = std::max(max,energize_from({x,0},direction::down));
		max = std::max(max,energize_from({x,height-1},direction::up));
	}
